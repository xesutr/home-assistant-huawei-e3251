...
class E3251Coordinator(DataUpdateCoordinator):
    """Data Update Coordinator for Huawei E3251 HiLink GSM Module with SQLite backend."""
    ...
    def _init_db(self) -> None:
        ...
        try:
            ...
            _LOGGER.error("E3251: SQLite initialization error: %s", e)
    ...
    def _sync_load_sms_from_db(self) -> list[dict]:
        ...
        try:
            ...
            _LOGGER.error("E3251: SQLite data read error: %s", e)
        return history
    ...
    def _sync_insert_sms_to_db(self, messages: list[dict]) -> None:
        ...
        try:
            ...
            _LOGGER.error("E3251: SQLite save error: %s", e)
    ...
    def _sync_delete_sms_from_db(self, sender: str, timestamp: str) -> None:
        ...
        try:
            ...
            _LOGGER.error("E3251: SQLite deletion error: %s", e)
    ...
    def _sync_delete_sms_by_id_from_db(self, sms_id: int) -> None:
        ...
        try:
            ...
            _LOGGER.error("E3251: SQLite ID deletion error: %s", e)
    ...
    async def _async_update_data(self) -> dict:
        ...
        # Modemden sinyal seviyesini çek
        signal_percent = await self.modem.async_get_signal_strength()
        ...
        return {
            ...
            "signal": signal_percent,  # sensor.py tarafından okunan anahtar
        }

    async def _update_new_sms(self) -> None:
        """Read unread SMS messages, assemble multipart messages, update history, write to DB, and clear SIM/modem memory."""
        # Modemden okunmamış SMS'leri async HTTP çağrısıyla al
        sms_list = await self.modem.async_read_unread_sms()
        ...
        if processed_messages:
            ...
            if new_records_to_save:
                ...
                # Clear modem memory after successful storage
                await self.modem.async_delete_all_read_sms(sms_list)

    async def async_delete_sms_by_id(self, sms_id: int) -> None:
        """Delete selected SMS by ID from both SQLite database and RAM."""
        try:
            target_id = int(sms_id)
        except (ValueError, TypeError):
            _LOGGER.error("E3251: Invalid SMS ID format for deletion: %s", sms_id)
            return

        await self.hass.async_add_executor_job(self._sync_delete_sms_by_id_from_db, target_id)

        # RAM'den silerken ID filtrelemesi
        self.sms_history = [
    ...
    async def async_delete_sms(self, sender: str, timestamp: str) -> None:
        """Delete selected SMS from both SQLite (modem.db) database and RAM (Fallback method)."""
        # 1. Delete from SQLite
        await self.hass.async_add_executor_job(self._sync_delete_sms_from_db, sender, timestamp)
