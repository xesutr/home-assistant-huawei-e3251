...
@dataclass
class E3251SensorConfigBase(Generic[T]):
...
SENSOR_TYPES = (
    ...
    E3251SensorConfigBase(
        value=lambda data: data.get("signal", 0) if data else 0,
        description=SensorEntityDescription(
            key="signal_strength",
...
async def async_setup_entry(
    ...
    """Set up E3251 sensors based on a config entry."""
    coordinator: E3251Coordinator = hass.data[DOMAIN][entry.entry_id]

    sensors = [
        E3251Sensor(coordinator, sensor_config)
...
class E3251Sensor(CoordinatorEntity[E3251Coordinator], SensorEntity):
    """E3251 Sensor Entity architecture fully compatible with Home Assistant standard."""

    _attr_has_entity_name = True

    def __init__(
        self,
        coordinator: E3251Coordinator,
        sensor_config: E3251SensorConfigBase,
    ) -> None:
